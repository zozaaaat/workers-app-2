...
const LeavesPage: React.FC = () => {
  ...
  const [filter, setFilter] = useState({ status: '', leave_type: '' });
  ...
  const fetchLeaves = async () => {
    ...
    try {
      ...
      setError(t('general.load_error') || "تعذر تحميل الإجازات");
    } finally {
      setLoading(false);
    }
  };
  ...
  // فلترة متقدمة
  const advancedFilteredLeaves = leaves.filter(l =>
    (l.worker_name.includes(search) || (l.company || '').includes(search)) &&
    (filter.status ? l.status === filter.status : true) &&
    (filter.leave_type ? l.leave_type === filter.leave_type : true)
  );
  // طباعة
  const handlePrint = () => {
    ...
    if (printWindow) {
      printWindow.document.write('<html><head><title>' + t('sidebar.leaves') + '</title>');
  ...
  return (
    <Box sx={{ p: 2 }}>
      <Typography variant="h5" mb={2}>{t('sidebar.leaves')}</Typography>
      <Box display="flex" flexWrap="wrap" gap={2} mb={2}>
        ...
        <TextField
          ...
          onChange={e => setFilter(f => ({ ...f, leave_type: e.target.value }))}
          sx={{ width: 150 }}
        />
        <Box flexGrow={1} />
        <Box>
      ...
      <Paper>
            ...
            <Table>
              <TableHead>
                <TableRow>
                  ...
                  <TableCell>{t('general.leave_type')}</TableCell>
                  <TableCell>{t('general.start_date')}</TableCell>
                  <TableCell>{t('general.end_date')}</TableCell>
              ...
              <TableBody>
                ...
                ) : advancedFilteredLeaves.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map(leave => (
                  <TableRow key={leave.id} hover onClick={() => handleRowClick(leave)} style={{ cursor: 'pointer' }}>
                    <TableCell>{leave.worker_name}</TableCell>
                    <TableCell>{leave.company}</TableCell>
                    ...
                    <TableCell>
                      {role === "admin" || role === "employee" ? (
                        <>
                          <IconButton color="primary" onClick={e => { e.stopPropagation(); handleOpenDialog(leave); }}><EditIcon /></IconButton>
                          <IconButton color="error" onClick={e => { e.stopPropagation(); handleDelete(leave.id); }}><DeleteIcon /></IconButton>
                        </>
                      ) : null}
                    </TableCell>
                  </TableRow>
        ...
        <TablePagination
          component="div"
          count={advancedFilteredLeaves.length}
          page={page}
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog} maxWidth="sm" fullWidth>
        <DialogTitle>{editLeave ? t('general.edit') : t('general.add')}</DialogTitle>
        <DialogContent>
          <TextField
            margin="dense"
          ...
          <TextField
            margin="dense"
            label={t('general.leave_type') || 'نوع الإجازة'}
            name="leave_type"
          ...
          <TextField
            margin="dense"
            label={t('general.start_date') || 'تاريخ البداية'}
            name="start_date"
            type="date"
          ...
          <TextField
            margin="dense"
            label={t('general.end_date') || 'تاريخ النهاية'}
            name="end_date"
            type="date"
            value={form.end_date || ""}
            onChange={handleFormChange}
      ...
      <Drawer anchor="left" open={detailsOpen} onClose={handleCloseDetails} sx={{ zIndex: 1301 }}>
        <Box sx={{ width: 350, p: 3 }}>
          <Typography variant="h6" mb={2}>{t('sidebar.leaves')} - {selectedLeave?.worker_name}</Typography>
          <Divider sx={{ mb: 2 }} />
          <List>
            <ListItem><ListItemText primary={t('general.company') + ': ' + (selectedLeave?.company || '')} /></ListItem>
            <ListItem><ListItemText primary={t('general.leave_type') + ': ' + (selectedLeave?.leave_type || '')} /></ListItem>
            <ListItem><ListItemText primary={t('general.start_date') + ': ' + (selectedLeave?.start_date || '')} /></ListItem>
            <ListItem><ListItemText primary={t('general.end_date') + ': ' + (selectedLeave?.end_date || '')} /></ListItem>
            <ListItem><ListItemText primary={t('general.status') + ': ' + (selectedLeave?.status || '')} /></ListItem>
            <ListItem><ListItemText primary={t('general.notes') + ': ' + (selectedLeave?.notes || '')} /></ListItem>
          </List>
