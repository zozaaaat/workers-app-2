...
const LoginPage: React.FC = () => {
  ...
  return (
    <Box minHeight="100vh" display="flex" alignItems="center" justifyContent="center"
      sx={{
        background: `linear-gradient(rgba(123,47,242,0.7), rgba(40,150,255,0.7))`,
      ...
      <Paper elevation={8} className="login-paper-animate" sx={{
        ...
        <form onSubmit={handleLogin}>
          <TextField aria-label="input field" label="اسم المستخدم" fullWidth margin="normal" value={username} onChange={e => setUsername(e.target.value)} required
            sx={{ input: { background: '#f3f6fd', borderRadius: 2 } }}
          />
          <TextField aria-label="input field" label="كلمة المرور" type="password" fullWidth margin="normal" value={password} onChange={e => setPassword(e.target.value)} required
            sx={{ input: { background: '#f3f6fd', borderRadius: 2 } }}
          />
          {error && (
